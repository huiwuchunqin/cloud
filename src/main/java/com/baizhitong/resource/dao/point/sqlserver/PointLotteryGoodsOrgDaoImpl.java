...
/**
 * 机构商品dao实现 PointLotteryGoodsOrgDaoImpl TODO
 * 
 * @author creator BZT 2016年6月21日 下午7:29:20
 * @author updater
 *
 * @version 1.0.0
 */
@Service
public class PointLotteryGoodsOrgDaoImpl extends BaseSqlServerDao<PointLotteryGoodsOrg>
                implements PointLotteryGoodsOrgDao {
    /**
     * 新增 ()<br>
     * 
     * @param goods
     * @return
     */
    public boolean add(PointLotteryGoodsOrg goods) {
        try {
     ...
     * @return
     */
    public Page getList(String orgCode, String goodsName, Integer role, Integer page, Integer rows) {
        String sql = "select * from point_lottery_goods_org where 1=1 and flagDelete=0 ";
    ...
    public int deleteGood(Integer id) {
        String sql = " update  point_lottery_goods_org set flagDelete=1 where id=?";
    ...
    public double getLeftProbality(Integer role, String orgCode) {
        ...
        sql.append("    FROM");
        sql.append("        point_lottery_goods_org ");
        sql.append("        ");
        sql.append(" where orgCode=:orgCode and flagDelete=0 and flagUsing=1 ");
     ...
     * @return
     */
    public int getCount(int role, String orgCode) {
        StringBuffer sql = new StringBuffer();
        Map<String, Object> sqlParam = new HashMap<String, Object>();
        sql.append("SELECT");
        ...
        sql.append("     and   userRoleRequirement =:role  ");
        sql.append("        AND flagUsing = 1 and orgCode=:orgCode and flagDelete=0 ");
        sqlParam.put("role", role);
        sqlParam.put("orgCode", orgCode);
        Map<String, Object> map = super.findUniqueBySql(sql.toString(), sqlParam);
        if (map != null) {
