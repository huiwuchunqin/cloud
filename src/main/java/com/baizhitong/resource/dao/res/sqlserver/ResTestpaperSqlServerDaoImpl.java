...
/**
 * 资源_试卷数据SqlServer接口实现
 * 
 * @author creator cuidc 2015/12/03
 * @author updater
 */
@Service("resTestpaperSqlServerDao")
public class ResTestpaperSqlServerDaoImpl extends BaseSqlServerDao<ResTestpaper> implements ResTestpaperDao {
    ...
    public Integer saveTestpaperInfo(ResTestpaper resTestpaper) throws Exception {
        return MapUtils.getInteger(super.saveAndReturnId(resTestpaper), "id");
    }
    ...
    public Map<String, Object> selectQuestionTypeSubject(String sectionCode, String subjectCode,
        ...
        StringBuffer sql = new StringBuffer();
        sql.append(" SELECT * ");
        sql.append(" FROM res_question_type_subject rqts ");
        sql.append(" WHERE rqts.sectionCode = :sectionCode AND rqts.subjectCode = :subjectCode AND rqts.questionTypeCode = :questionTypeCode ");

        // SQL参数
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("sectionCode", sectionCode);
        map.put("subjectCode", subjectCode);
        map.put("questionTypeCode", questionTypeCode);
