function checkJatoolsPrinterInstalled() {
	...
	if (!k) {
		...
		if (navigator.userAgent.indexOf("Chrome") > -1) {
			...
			if (!m) {
				i = "�ܱ��ӡ�ؼ�δ��װ������<a href='jatoolsPrinter.crx'>�˴�</a>��װ."
			}
		}
	}
	if (i) {
...
function viewSource() {
	...
	window
			.showModalDialog(
					"/sourceviewer/view.jsp?from=" + escape(b),
					null,
...
function JP(p) {
	...
	function u(c) {
		var a = [ "border", "border-radius", "box-shadow", "height", "margin",
				"padding", "width", "max-width", "min-width",
	...
	function l(a) {
		var b = "<html><head><style>" + v(a.ownerDocument)
				+ "</style></head><body>" + n(a.ownerDocument, a)
				+ "</body></html>";
		return b
	}
	function s(a) {
		var b = "<html><head><base href='" + a.URL + "'/><style>" + v(a)
				+ "</style></head><body>" + a.body.innerHTML + "</body></html>";
		return b
	}
	function q() {
	...
	return ({
		...
		initialize : function() {
			...
			if (!p) {
				...
				window.addEventListener("message", function(b) {
					a.callbacks[b.data.event].apply(null, b.data.params
							|| [ b.data.data ])
				})
			}
			return this
		...
		sendMessage : function(b, a) {
			/*chrome.runtime.sendMessage(this.extension, b, a
					|| this.emptyCallback)*/
		},
		about : function() {
		...
		isCustomPaperSupported : function(b, a) {
			if (this.proxy) {
				(a || this.nothing).call(this, this.proxy
						.isCustomPaperSupported(b))
			} else {
				this.sendMessage({
		...
		exportAsExcel : function(b, e, d, c) {
			if (this.proxy) {
				(c || this.nothing).call(this, this.proxy
						.exportAsExcel(b, e, d))
			} else {
				var a = l(b);
				this.sendMessage({
		...
		showPageSetupDialog : function(a) {
			if (this.proxy) {
				(a || this.nothing)
						.call(this, this.proxy.showPageSetupDialog())
			} else {
				this.sendMessage({
		...
		setLastSettings : function(b, a, c) {
			if (this.proxy) {
				(c || this.nothing)
						.call(this, this.proxy.setLastSettings(b, a))
			} else {
				this.sendMessage({
		...
		setPrintBackground : function(b, a) {
			if (this.proxy) {
				(a || this.nothing)
						.call(this, this.proxy.setPrintBackground(b))
			} else {
				this.sendMessage({
...
function getJatoolsPrinter(c) {
	if (!_jp) {
		var c = c || document;
		var d = navigator.userAgent.match(/(msie\s|trident.*rv:)([\w.]+)/i) ? c
				.getElementById("ojatoolsPrinter") : c
...
function declareJatoolsPrinter(c) {
	...
	var d = '<object id="ojatoolsPrinter" '
			+ e
			+ 'codebase="jatoolsPrinter.cab#version=5,4,0,0" classid="clsid:B43D3361-D075-4BE2-87FE-057188254255" width="0" height="0">';
	if (!isChrome45()) {
