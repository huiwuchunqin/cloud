...
function zTreeOnAsyncSuccess(event, treeId, treeNode, msg) {
	if (treeNode) {
		var zTree = $.fn.zTree.getZTreeObj("knowledgePointNodeInfo");
		zTree.expandAll(true);
		_.each(eval(tagResRelList), function(tagRel, index) {

			var ztreeNode = zTree.getNodesByParam('id', tagRel.tagContentID)[0];
			$.fn.zTree.getZTreeObj("knowledgePointNodeInfo").selectNode(ztreeNode, true);
			// $.fn.zTree.getZTreeObj("knowledgePointNodeInfo").click();
		});

		/*
		 * if (typeof (treeNode.callBack) == "function") { treeNode.callBack(); }
		 * if (typeof (treeNode.callBack) == "object") {
		 * $.each(treeNode.callBack, function(i, v) { if (typeof (v) ==
		 * "function") { v(); } }) }
		 */
	}
}
...
function reverseKnowledegSelect(data, zTree) {
	_.each(data, function(tagRel, index) {
		...
		zte_tagSet.zAsync = true;

		/** 加载完tagSettings后回调 */
		/*
		 * if(!zte_tagSet||!zte_tagSet.callBack||!zte_tagSet.callBack.length){
		 * zte_tagSet.callBack=[]; } zte_tagSet.callBack.push( function(){ var
		 * zte_tagRel=getTagContentById(tagRel.tagContentID);
		 * switchFartherZtreeNode(zte_tagRel); selectNode(zte_tagRel,true); } );
		 * 
		 * switchNode(zte_tagSet.tId,true);
		 */// 异步加载tagContent
		// zTree.expandNode(zte_tagSet, true, true, true);
		/*
		 * if (!zte_tagSet || !zte_tagSet.callBack ||
		 * !zte_tagSet.callBack.length) { zte_tagSet.callBack = []; }
		 * zte_tagSet.callBack.push(function() { var zte_tagRel =
		 * getTagContentById(tagRel.tagContentID);
		 * switchFartherZtreeNode(zte_tagRel); selectNode(zte_tagRel, true); });
		 */
		zTree.reAsyncChildNodes(zte_tagSet, "", false);
	});
	function getTagContentById(id) {
		var tagContent = null;
		var ztreeNodes = zTree.getNodesByParam('id', id);
...
function TextbookRelzTreeClick(zTree) {
	...
	$.each(checkedNodes, function(i, node) {
		var fullName = "", textbookName = "", publisherName = "";

		var _thisNode = $("#" + node.tId), _textbook = null, _publisher = null;
 ...
 * 知识点树
 */
function knowledgezTreeClick(zTree, initData) {
	var checkedNodes = [];
	var ary = [];
	// 获取所有被选中的最底层节点
	_.each(zTree.getSelectedNodes(), function(checkedNode, index) {
	...
	_.each(checkedNodes, function(checkedNode) {
		var obj = $.extend({}, initData, {
			tagID : checkedNode.tagID,
			tagCode : checkedNode.tagCode,
			tagContentId : checkedNode.id,
			tagContentCode : checkedNode.contentCode,
			name : checkedNode.name,
			textBookVersionCode : checkedNode.textBookVersionCode,
			tagSettingsId : checkedNode.tagSettingsId
		});
		ary.push(obj);
	});
	return ary;
...
function initKnowledgeTree(resType, sectionCode, subjectCode, gradeCode, publisherCode) {
	if (publisherCode == undefined || publisherCode == "") {
		publisherCode = -1;
	}
	...
	var data = {
		resType : resType,
		sectionGlobalCode : sectionCode,
		subjectGlobalCode : subjectCode,
		gradeGlobalCode : gradeCode,
		textBookVersionCode : publisherCode
	}
	$.fn.zTree.init($("#knowledgePoint"), knowledgeTreeSetting, getKnowledeg_initData(data));
}
